...
desinstalar_postgres () {
    ...
    echo "$password" | sudo -S rm -r /var/lib/etc/postgresql
    echo "$password" | sudo -S usedel -r postgres
    echo "$password" | sudo -S groupdel rm -r postgresql
    read -n 1 -s -r -p "PRESIONE [ENTER] para continuar..."
}
...
sacar_respaldo () {
    ...
    read -p "Elegir laa base de datos a respaldar:" bddRespaldo
    echo -e "\n" 
    if [ -d "$1" ]; then
...
do
    ...
    case $opcion in 
        ...
        4)          
            ...
            restaurar_respaldo $directorioRepaldos
            sleep 3
            ;;
        5) 
            exit 0
